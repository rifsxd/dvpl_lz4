...
func Cli() {
	...
	case "verify":
		...
		if err != nil {
			...
			log.Printf("%s%s FINISHED%s. Successful verifications: %s%d%s, Failed verifications: %s%d%s, Ignored files: %s%d%s", colors.GreenColor, strings.ToUpper(config.Mode), colors.ResetColor, colors.GreenColor, successCount, colors.ResetColor, colors.RedColor, failureCount, colors.ResetColor, colors.YellowColor, ignoredCount, colors.ResetColor)
		}
	case "gui":
